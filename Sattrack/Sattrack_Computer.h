...
class Sattrack_Computer {
  private:
    ...
    Sattrack_Memory _memory = Sattrack_Memory();
    Sattrack_Clock _clock = Sattrack_Clock();
    enum { NOUN, VERB, PROG, LN_0, LN_1, LN_2, NONE };
    uint32_t _buffer[3];
    uint8_t _word[3] = {1, 13, 0};
    uint8_t _mode = NONE;
    uint8_t _i;
    char _scr_buffer[9];
    char _scr_word[3];
    ...
    void update() {
      _memory.set(1, _clock.date());
      _memory.set(2, _clock.time());
      _memory.set(3, millis());

      if (11 <= _word[VERB] && _word[VERB] <= 13) {
        for (_i = 0; _i <= _word[VERB] - 11; _i++) {
          _display.field(_i + 3, _memory.get(_word[NOUN] + _i));
        }
      }
    }
